...
def generate_data(dataset, key):
    if dataset in ['avoidance_recursive_hc3', 'hc3_stylisticCleanup']:
        ...
        sampled = [strip_newlines(x.strip()) for x in paired_data['sampled'] if isinstance(x, str)]

        paired = []
        for o, s in zip(originals, sampled):
            if not o or not s:
                continue
            o, s = trim_to_shorter_length(o, s)
            if o and s:
        ...
        # Keep only paired examples with <= 512 tokens for mask model compatibility.
        original_tokenized = preproc_tokenizer([x[0] for x in paired])
        sampled_tokenized = preproc_tokenizer([x[1] for x in paired])

        filtered_pairs = []
        for pair, o_ids, s_ids in zip(paired, original_tokenized['input_ids'], sampled_tokenized['input_ids']):
            if len(o_ids) <= 512 and len(s_ids) <= 512:
                filtered_pairs.append(pair)

        data = {
            'original': [x[0] for x in filtered_pairs[:n_samples]],
            'sampled': [x[1] for x in filtered_pairs[:n_samples]],
        }

        print(f"Total number of paired samples: {len(data['original'])}")
        print(f"Average number of words (original): {np.mean([len(x.split()) for x in data['original']])}")
        print(f"Average number of words (sampled): {np.mean([len(x.split()) for x in data['sampled']])}")
...
def eval_supervised(data, model):
    ...
    # free model memory; clear CUDA cache only when CUDA is available
    del detector
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="avoidance_recursive_hc3")
